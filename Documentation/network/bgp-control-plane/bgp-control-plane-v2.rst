...
In addition to that, ToR switches need to be configured with "bgp listen range" for dynamic bgp neighors. It will ensure that the ToR switch accepts the BGP session from cilium nodes
by listening to a specific prefix range. So ToRs do not need to know the exact peer address of the cilium node.
Moreover, the ToR switch should also be configured with local ASN so that we can use it as a consistent peer ASN in Cilium BGP configuration.
